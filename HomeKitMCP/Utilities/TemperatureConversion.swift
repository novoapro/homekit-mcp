...
enum TemperatureConversion {
    ...
    // MARK: - Workflow Migration

    /// Migrates all workflow temperature values when the unit preference changes.
    /// `convert` is the conversion function (e.g., C→F or F→C).
    static func migrateWorkflows(
        workflowStorage: WorkflowStorageService,
        registry: DeviceRegistryService?,
        convert: @escaping (Double) -> Double
    ) async {
        let workflows = await workflowStorage.getAllWorkflows()
        for workflow in workflows {
            await workflowStorage.updateWorkflow(id: workflow.id) { w in
    ...
    private static func migrateTrigger(_ trigger: WorkflowTrigger, registry: DeviceRegistryService?, convert: @escaping (Double) -> Double) -> WorkflowTrigger {
        switch trigger {
    ...
    private static func migrateCondition(_ condition: WorkflowCondition, registry: DeviceRegistryService?, convert: @escaping (Double) -> Double) -> WorkflowCondition {
        switch condition {
    ...
    private static func migrateBlock(_ block: WorkflowBlock, registry: DeviceRegistryService?, convert: @escaping (Double) -> Double) -> WorkflowBlock {
        switch block {
    ...
    private static func migrateAction(_ action: WorkflowAction, registry: DeviceRegistryService?, convert: @escaping (Double) -> Double) -> WorkflowAction {
        switch action {
    ...
    private static func migrateFlowControl(_ fc: FlowControlBlock, registry: DeviceRegistryService?, convert: @escaping (Double) -> Double) -> FlowControlBlock {
        ...
        case .delay, .waitForState, .stop, .executeWorkflow:
            return fc
        }
    }
